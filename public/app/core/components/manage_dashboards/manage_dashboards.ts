...
export class ManageDashboardsCtrl {
  ...
  starredFilterOptions = [{ text: '按星标筛选', disabled: true }, { text: '是' }, { text: '否' }];
  ...
  delete() {
    ...
    if (folderCount > 0 && dashCount > 0) {
      text += `选择的文件夹${folderCount === 1 ? '' : 's'} 和仪表盘${dashCount === 1 ? '' : 's'}?`;
      text2 = `所选文件夹内所有仪表板${folderCount === 1 ? '' : 's'} 也会被删除`;
    } else if (folderCount > 0) {
      text += `所选文件夹${folderCount === 1 ? '' : 's'} 及其所有仪表板？`;
    } else {
      text += `所选仪表盘${dashCount === 1 ? '' : 's'}?`;
    }
  ...
  initTagFilter() {
    return this.searchSrv.getDashboardTags().then(results => {
      this.tagFilterOptions = [{ term: '按星标筛选', disabled: true }].concat(results);
