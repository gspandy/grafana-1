...
func (pm *PluginManager) updateAppDashboards() {
	pm.log.Debug("寻找App Dashboard更新")
...
func autoUpdateAppDashboard(pluginDashInfo *PluginDashboardInfoDTO, orgId int64) error {
	...
	plog.Info("自动更新App仪表板", "dashboard", dash.Title, "newRev", pluginDashInfo.Revision, "oldRev", pluginDashInfo.ImportedRevision)
...
func syncPluginDashboards(pluginDef *PluginBase, orgId int64) {
	plog.Info("将插件仪表板同步到DB", "pluginId", pluginDef.Id)
	...
	for _, dash := range dashboards {
		...
		if dash.Removed {
			...
			if err := bus.Dispatch(&deleteCmd); err != nil {
				plog.Error("无法自动更新 app 仪表盘", "pluginId", pluginDef.Id, "error", err)
				return
			}
