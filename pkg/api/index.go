...
func (hs *HTTPServer) setIndexViewData(c *m.ReqContext) (*dtos.IndexViewData, error) {
	...
	if hasEditPermissionInFoldersQuery.Result {
		...
		if c.OrgRole == m.ROLE_ADMIN || c.OrgRole == m.ROLE_EDITOR {
			children = append(children, &dtos.NavLink{Text: "文件夹", SubTitle: "创建新的文件夹以管理组织", Id: "folder", Icon: "gicon gicon-folder-new", Url: setting.AppSubUrl + "/dashboards/folder/new"})
		}

		children = append(children, &dtos.NavLink{Text: "导入", SubTitle: "从文件或Grafana.com导入新的仪表盘", Id: "import", Icon: "gicon gicon-dashboard-import", Url: setting.AppSubUrl + "/dashboard/import"})
	...
	dashboardChildNavs := []*dtos.NavLink{
		...
		{Text: "分频器", Divider: true, Id: "divider", HideFromTabs: true},
	...
	if setting.ExploreEnabled && (c.OrgRole == m.ROLE_ADMIN || c.OrgRole == m.ROLE_EDITOR) {
		data.NavTree = append(data.NavTree, &dtos.NavLink{
			...
			Children: []*dtos.NavLink{
				{Text: "新标签", Icon: "gicon gicon-dashboard-new", Url: setting.AppSubUrl + "/explore"},
			},
		})
	}
	...
	for _, plugin := range enabledPlugins.Apps {
		if plugin.Pinned {
			...
			if len(appLink.Children) > 0 && c.OrgRole == m.ROLE_ADMIN {
				...
				appLink.Children = append(appLink.Children, &dtos.NavLink{Text: "插件配置", Icon: "gicon gicon-cog", Url: setting.AppSubUrl + "/plugins/" + plugin.Id + "/edit"})
			}
		...
		if c.IsGrafanaAdmin {
			cfgNode.Children = append(cfgNode.Children, &dtos.NavLink{
				...
				Children: []*dtos.NavLink{
					...
					{Text: "S设置", Id: "server-settings", Url: setting.AppSubUrl + "/admin/settings", Icon: "gicon gicon-preferences"},
